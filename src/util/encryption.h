...
public:
	struct EncryptedData
	{
		...
		{
			if (raw_data.size() < sizeof(id) + salt_size + mac_size + sizeof(uint64_t) +
										  sizeof(uint64_t))
				return false;
			...
			if (raw_data.size() <
					sizeof(id) + salt_size + mac_size + sizeof(uint64_t) + filename_size)
				return false;

			filename.clear();
			filename.append(
					&raw_data[sizeof(id) + salt_size + mac_size + sizeof(uint64_t)],
					filename_size);
			...
			if (raw_data.size() < sizeof(id) + salt_size + mac_size + sizeof(uint64_t) +
										  filename_size + sizeof(uint64_t) + data_size)
				return false;

			data.clear();
			data.append(&raw_data[sizeof(id) + salt_size + mac_size + sizeof(uint64_t) +
